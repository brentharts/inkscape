...
void AttrDialog::onAttrDelete(Glib::ustring path)
{
    ...
    if (row) {
        ...
        if (name == "content") {
            ...
            this->_repr->setAttribute(name, nullptr);
...
void AttrDialog::nameEdited (const Glib::ustring& path, const Glib::ustring& name)
{
    ...
    if(row && this->_repr) {
        ...
        if (!old_name.empty()) {
            ...
            _updating = true;
            _repr->setAttribute(old_name, nullptr);
            _updating = false;
        }
        ...
        grab_focus();
        _updating = true;
        _repr->setAttribute(name, value); // use char * overload (allows empty attribute values)
        _updating = false;
        g_timeout_add(50, &sp_attrdialog_store_move_to_next, this);
