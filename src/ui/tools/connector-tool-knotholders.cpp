...
ConnectorObjectKnotHolder::ConnectorObjectKnotHolder(SPDesktop *desktop, SPItem *item,
                                                     SPKnotHolderReleasedFunc relhandler)
...
ConnectorLineKnotHolder::ConnectorLineKnotHolder(SPDesktop *desktop, SPItem *item, SPKnotHolderReleasedFunc relhandler)
    : KnotHolder(desktop, item, relhandler)
    , i2dt(desktop->getDocument()->getRoot()->i2dt_affine())
{
    ...
    auto route_pathv = *(lpe->getRoutePath());
    ...
    if (original_pathv.size() == 1) {
        ...
        if (lpe->advancedEditor()) {
            ...
            for (int i = 1; i < original_pathv[0].size(); i++) {
                ...
                int dynamic = LPEConnectorLine::getCheckpointDynamic(&(original_pathv[0][i-1]), &(original_pathv[0][i]));
...
Geom::Path ConnectorLineKnotHolder::getNewRoutePath()
{
    ...
    for (const auto & i : advanced_path) {
        path.append(i);
    }
...
void ConnectorLineKnotHolder::commitAdvancedLine()
{
    ...
    for (int i = 1; i < route_path.size() - 1; i++) {
        pathv.back().appendNew<Geom::LineSegment>(
            Geom::lerp(0.5, path[i].initialPoint(), path[i].finalPoint())
        );
    }
...
ConnectorLineKnotHolder::~ConnectorLineKnotHolder()
{
}
...
Geom::Point ConnectorKnotMidpoint::knot_get() const
{
    ...
    return Geom::lerp(0.5, path[_index].initialPoint(), path[_index].finalPoint());
}

Geom::Point ConnectorKnotMidpoint::moveOneAxis(bool vert, Geom::Point origin, Geom::Point raw)
{
    if (vert)
        return Geom::Point(raw[Geom::X], origin[Geom::Y]);
    return Geom::Point(origin[Geom::X], raw[Geom::Y]);
}

// Update the route line in the parent as we re-draw it
void ConnectorKnotMidpoint::knot_set(Geom::Point const &raw, Geom::Point const &origin, unsigned int state)
{
...
void ConnectorKnotMidpoint::knot_ungrabbed(Geom::Point const &p, Geom::Point const &origin, guint state)
{
    holder()->commitAdvancedLine();
}


ConnectorKnotCheckpoint::ConnectorKnotCheckpoint(int index, int dir, int dynamic)
    : ConnectorKnot()
    , index(index)
    , dir(dir)
    , dynamic(dynamic)
...
void ConnectorKnotCheckpoint::knot_set(Geom::Point const &raw, Geom::Point const &origin, unsigned int state)
{
    auto snap_point = snap_knot_position(raw, state);
    ...
    if (tolerance > 0) {
        ...
        if (dynamic & DynamicX && fabs(snap_point[Geom::X] - origin[Geom::X]) > tolerance)
            dynamic -= DynamicX;
        if (dynamic & DynamicY && fabs(snap_point[Geom::Y] - origin[Geom::Y]) > tolerance)
            dynamic -= DynamicY;
    }
...
bool ConnectorKnotCheckpoint::knot_event(Inkscape::CanvasEvent const &event)
{
    if (event.type() == EventType::BUTTON_PRESS) {
        ...
        if (button_event.button == 1 && button_event.num_press == 2) {
            ...
            LPEConnectorLine::rewriteLine(cast<SPShape>(item), index, Geom::Point(), 0, DynamicNone, RewriteDelete);
