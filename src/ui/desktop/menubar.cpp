...
build_menu()
{
    ...
    if (!gmenu) {
        ...
        { // Recent file
            ...
            std::vector<Glib::RefPtr<Gtk::RecentInfo>> inkscape_recents;
            ...
            for (auto const &recent_file : recent_files) {
                ...
                inkscape_recents.push_back(recent_file);
            }

            // Sort the recent files by how recent they are
            std::sort(inkscape_recents.begin(), inkscape_recents.end(),
                      [](Glib::RefPtr<Gtk::RecentInfo> a, Glib::RefPtr<Gtk::RecentInfo> b) -> bool {
                return a->get_visited() > b->get_visited();
            });

            // Escape the underscores to prevent them from being interpreted as accelerator mnemonics
            ...
            for (auto &recent_file : inkscape_recents) {
