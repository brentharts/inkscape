...
ShapeEditor::ShapeEditor(SPDesktop *dt, Geom::Affine edit_transform, bool edit_marker_mode) :
    desktop(dt),
    knotholder(nullptr),
...
void ShapeEditor::set_item(SPItem *item) {
    ...
    if (item) {
        ...
        if (!this->knotholder) {
            ...
            this->knotholder = createKnotHolder(item, desktop, _edit_marker_mode);
        }
        ...
        if (this->knotholder) {
            ...
            this->knotholder->setEditMarkerMode(_edit_marker_mode);
            this->knotholder->update_knots();
