...
void SelectToolbar::setDesktop(SPDesktop *desktop)
{
    if (_desktop) {
        ...
        _selectionModifiedConn.disconnect();
    }
    ...
    if (_desktop) {
        ...
        _selectionChangedConn = sel->connectChanged(sigc::mem_fun(*this, &SelectToolbar::_selectionChanged));
        _selectionModifiedConn = sel->connectModified(sigc::mem_fun(*this, &SelectToolbar::_selectionModified));

        // Update now.
        layout_widget_update(sel);
        _sensitize();

        for (auto btn : {&_x_item, &_y_item, &_w_item, &_h_item}) {
