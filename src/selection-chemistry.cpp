...
static void selection_display_message(SPDesktop *desktop, Inkscape::MessageType msgType, Glib::ustring const &msg)
{
    if (desktop) {
        ...
        if (msgType == Inkscape::IMMEDIATE_MESSAGE || msgType == Inkscape::WARNING_MESSAGE ||
...
void SelectionHelper::fixSelection(SPDesktop *dt)
{
    ...
    for (auto i = boost::rbegin(selList); i != boost::rend(selList); ++i) {
        SPItem *item = *i;
        if (item && !dt->isLayer(item) && (!item->isLocked())) {
 ...
 */
static void sp_selection_copy_one(Inkscape::XML::Node *repr, Geom::Affine full_t,
                                  std::vector<Inkscape::XML::Node *> &clip, Inkscape::XML::Document *xml_doc)
{
...
static void sp_selection_copy_impl(std::vector<SPItem *> const &items, std::vector<Inkscape::XML::Node *> &clip,
...
static void sp_selection_delete_impl(std::vector<SPItem *> const &items, bool propagate = true,
    ...
    if (desktop()) {
        if (dynamic_cast<TextTool *>(desktop()->event_context)) {
            if (Inkscape::UI::Tools::sp_text_delete_selection(desktop()->event_context)) {
                DocumentUndo::done(desktop()->getDocument(), SP_VERB_CONTEXT_TEXT, _("Delete text"));
                return;
            }
        }
...
void ObjectSet::duplicate(bool suppressDone, bool duplicateLayer)
{
    ...
    auto text_refs = text_categorize_refs(
        doc, reprs.begin(), reprs.end(), static_cast<text_ref_t>(TEXT_REF_DEF | TEXT_REF_EXTERNAL | TEXT_REF_INTERNAL));
            ...
            if (use) {
                ...
                for (unsigned int j = 0; j < old_ids.size(); j++) {
                    if (!strcmp(orig->getId(), old_ids[j])) {
                        ...
                        // std::cout << id  << " old, its ori: " << orig->getId() << "; will relink:" << new_ids[i] << "
                        // to " << new_ids[j] << "\n";
                ...
                for (guint j = 0; j < old_ids.size(); j++) {
                    if (!strcmp(source_href, old_ids[j])) {
                        doc->getObjectById(new_ids[i])
                            ->firstChild()
                            ->setAttribute("xlink:href", Glib::ustring("#") + new_ids[j]);
                    }
                }
            } else if (path) {
    ...
    if (!suppressDone) {
        DocumentUndo::done(document(), SP_VERB_EDIT_DUPLICATE, _("Duplicate"));
    }
    if (!duplicateLayer)
        setReprList(newsel);
    else {
...
void sp_edit_clear_all(Inkscape::Selection *selection)
{
    ...
    DocumentUndo::done(doc, SP_VERB_EDIT_CLEAR_ALL, _("Delete all"));
}
 ...
 */
std::vector<SPItem *> &get_all_items(std::vector<SPItem *> &list, SPObject *from, SPDesktop *desktop, bool onlyvisible,
                                     bool onlysensitive, bool ingroups, std::vector<SPItem *> const &exclude)
{
    for (auto &child : from->children) {
        SPItem *item = dynamic_cast<SPItem *>(&child);
        if (item && !desktop->isLayer(item) && (!onlysensitive || !item->isLocked()) &&
...
static void sp_edit_select_all_full(SPDesktop *dt, bool force_all_layers, bool invert)
{
    ...
    PrefsSelectionContext inlayer =
        (PrefsSelectionContext)prefs->getInt("/options/kbselection/inlayer", PREFS_SELECTION_LAYER);
...
Inkscape::XML::Node *ObjectSet::group()
{
    ...
    for (auto current : p) {
        if (current->parent() == topmost_parent) {
            ...
            // At this point, current may already have no item, due to its being a clone whose original is already moved
            // away So we copy it artificially calculating the transform from its repr->attr("transform") and the parent
            // transform
            ...
            std::vector<Inkscape::XML::Node *> copied =
                sp_selection_paste_impl(doc, doc->getObjectByRepr(topmost_parent), temp_clip);
            if (!temp_clip.empty())
                temp_clip.clear();
    ...
    DocumentUndo::done(doc, SP_VERB_SELECTION_GROUP, C_("Verb", "Group"));
...
void ObjectSet::ungroup(bool skip_undo)
{
    ...
    if (boost::distance(groups()) == 0) {
        if (desktop())
            selection_display_message(desktop(), Inkscape::ERROR_MESSAGE,
    ...
    if (document() && !skip_undo)
        DocumentUndo::done(document(), SP_VERB_SELECTION_UNGROUP, _("Ungroup"));
}
    ...
    if (!group) {
        if (desktop())
            selection_display_message(
                desktop(), Inkscape::ERROR_MESSAGE,
...
void ObjectSet::raiseToTop(bool skip_undo)
{
    ...
    if (document() && !skip_undo) {
        DocumentUndo::done(document(), SP_VERB_SELECTION_TO_FRONT, _("Raise to top"));
    }
}
...
void ObjectSet::lowerToBottom(bool skip_undo)
{
    ...
    if (document() && !skip_undo) {
        DocumentUndo::done(document(), SP_VERB_SELECTION_TO_BACK, _("Lower to bottom"));
    }
}
...
void ObjectSet::stackUp(bool skip_undo)
{
    ...
    if (document() && !skip_undo)
        DocumentUndo::done(document(), SP_VERB_SELECTION_STACK_UP,
                           // TRANSLATORS: undo history: "stack up" means to raise an object of its ordinal position by
                           // 1
...
void ObjectSet::stackDown(bool skip_undo)
{
    ...
    if (document() && !skip_undo)
        DocumentUndo::done(document(), SP_VERB_SELECTION_STACK_DOWN,
                           // TRANSLATORS: undo history: "stack down" means to lower an object of its ordinal position
                           // by 1
...
SPCSSAttr *take_style_from_item(SPObject *object)
{
    ...
    if (!(dynamic_cast<SPText *>(object) || dynamic_cast<SPTSpan *>(object) || dynamic_cast<SPTRef *>(object) ||
          dynamic_cast<SPString *>(object))) {
...
void ObjectSet::pastePathEffect()
{
    ...
    if (cm->pastePathEffect(this)) {
        DocumentUndo::done(document(), SP_VERB_EDIT_PASTE_LIVEPATHEFFECT, _("Paste live path effect"));
    }
}
...
void ObjectSet::removeLPE()
{
    ...
    if (document())
        DocumentUndo::done(document(), SP_VERB_EDIT_REMOVE_LIVEPATHEFFECT, _("Remove live path effect"));
}
...
void ObjectSet::removeFilter()
{
    ...
    sp_repr_css_attr_unref(css);
    if (document())
        DocumentUndo::done(document(), SP_VERB_EDIT_REMOVE_FILTER, _("Remove filter"));
}
...
void ObjectSet::pasteSize(bool apply_x, bool apply_y)
{
    ...
    if (cm->pasteSize(this, false, apply_x, apply_y)) {
        DocumentUndo::done(document(), SP_VERB_EDIT_PASTE_SIZE, _("Paste size"));
    }
}
...
void ObjectSet::pasteSizeSeparately(bool apply_x, bool apply_y)
{
    ...
    if (cm->pasteSize(this, true, apply_x, apply_y)) {
        DocumentUndo::done(document(), SP_VERB_EDIT_PASTE_SIZE_SEPARATELY, _("Paste size separately"));
    }
}
    ...
    for (auto item : items) {
        if (item) {
            ...
            sp_item_group_ungroup_handle_clones(item,
                                                (oldparent->i2doc_affine()) * ((newparent->i2doc_affine()).inverse()));
        }
    }
}
...
void ObjectSet::toNextLayer(bool skip_undo)
{
    ...
    if (next) {
        ...
        next =
            Inkscape::next_layer(dt->currentRoot(), dt->currentLayer()); // Fixes bug 1482973: crash while moving layers
        std::vector<Inkscape::XML::Node *> copied;
        if (next) {
        ...
        if (!skip_undo) {
            DocumentUndo::done(dt->getDocument(), SP_VERB_LAYER_MOVE_TO_NEXT, _("Raise to next layer"));
        }
    } else {
        no_more = true;
    }
...
void ObjectSet::toPrevLayer(bool skip_undo)
{
    ...
    if (next) {
        ...
        sp_selection_change_layer_maintain_clones(items_copy, next);
        std::vector<Inkscape::XML::Node *> temp_clip;
        sp_selection_copy_impl(items_copy, temp_clip,
                               dt->doc()->getReprDoc()); // we're in the same doc, so no need to copy defs
        sp_selection_delete_impl(items_copy, false, false);
        next = Inkscape::previous_layer(dt->currentRoot(),
                                        dt->currentLayer()); // Fixes bug 1482973: crash while moving layers
        std::vector<Inkscape::XML::Node *> copied;
        if (next) {
            copied = sp_selection_paste_impl(dt->getDocument(), next, temp_clip);
        } else {
        ...
        if (next)
            dt->setCurrentLayer(next);
        if (!skip_undo) {
            DocumentUndo::done(dt->getDocument(), SP_VERB_LAYER_MOVE_TO_PREV, _("Lower to previous layer"));
        }
    } else {
        no_more = true;
    }
...
void ObjectSet::toLayer(SPObject *moveto, bool skip_undo)
{
    ...
    if (moveto) {
        ...
        if (!temp_clip.empty())
            temp_clip.clear();
        if (moveto && dt)
            dt->setCurrentLayer(moveto);
        if (!skip_undo) {
            DocumentUndo::done(document(), SP_VERB_LAYER_MOVE_TO, _("Move selection to layer"));
        }
    }
}
...
void ObjectSet::applyAffine(Geom::Affine const &affine, bool set_i2d, bool compensate, bool adjust_transf_center)
{
    if (isEmpty())
        return;
    ...
    for (auto l = items_copy.begin(); l != items_copy.end(); ++l) {
        ...
        bool transform_textpath_with_path =
            ((dynamic_cast<SPText *>(item) && item->firstChild() && dynamic_cast<SPTextPath *>(item->firstChild())) &&
        ...
        bool transform_flowtext_with_frame =
            flowtext && includes(flowtext->get_frame(nullptr)); // (only the first frame is checked so far)
        ...
        if (adjust_transf_center) { // The transformation center should not be touched in case of pasting or importing,
                                    // which is allowed by this if clause
...
void ObjectSet::removeTransform()
{
    ...
    if (document())
        DocumentUndo::done(document(), SP_VERB_OBJECT_FLATTEN, _("Remove transform"));
}
...
void ObjectSet::setScaleAbsolute(double x0, double x1, double y0, double y1)
{
    ...
    Geom::Scale const newSize(x1 - x0, y1 - y0);
...
void ObjectSet::setScaleRelative(Geom::Point const &align, Geom::Scale const &scale)
{
    ...
    // FIXME: ARBITRARY LIMIT: don't try to scale above 1 Mpx, it won't display properly and will crash sooner or later
    // anyway
    if (bbox->dimensions()[Geom::X] * scale[Geom::X] > 1e6 || bbox->dimensions()[Geom::Y] * scale[Geom::Y] > 1e6) {
        return;
    }
...
void ObjectSet::skewRelative(Geom::Point const &align, double dx, double dy)
{
    Geom::Translate const d2n(align);
    Geom::Translate const n2d(-align);
    Geom::Affine const skew(1, dy, dx, 1, 0, 0);
...
void ObjectSet::rotate90(bool ccw)
{
    ...
    Geom::Rotate const rot_90(Geom::Point(0, ccw ? -y_dir : y_dir)); // pos. or neg. rotation, depending on the value of
                                                                     // ccw
    for (auto l = items_copy.begin(); l != items_copy.end(); ++l) {
        SPItem *item = *l;
        if (item) {
    ...
    if (document())
        DocumentUndo::done(document(), ccw ? SP_VERB_OBJECT_ROTATE_90_CCW : SP_VERB_OBJECT_ROTATE_90_CW,
...
void ObjectSet::rotate(gdouble const angle_degrees)
{
    ...
    if (document())
        DocumentUndo::maybeDone(document(), ((angle_degrees > 0) ? "selector:rotate:ccw" : "selector:rotate:cw"),
                                SP_VERB_CONTEXT_SELECT, _("Rotate"));
}
...
void sp_select_same_fill_stroke_style(SPDesktop *desktop, gboolean fill, gboolean stroke, gboolean style)
{
    ...
    std::vector<SPItem *> all_list =
        get_all_items(x, desktop->currentRoot(), desktop, onlyvisible, onlysensitive, ingroups, y);
 ...
 */
std::vector<SPItem *> sp_get_same_fill_or_stroke_color(SPItem *sel, std::vector<SPItem *> &src,
...
static bool item_type_match(SPItem *i, SPItem *j)
{
    ...
    } else if (dynamic_cast<SPText *>(i) || dynamic_cast<SPFlowtext *>(i) || dynamic_cast<SPTSpan *>(i) ||
               dynamic_cast<SPTRef *>(i) || dynamic_cast<SPString *>(i)) {
        return (dynamic_cast<SPText *>(j) || dynamic_cast<SPFlowtext *>(j) || dynamic_cast<SPTSpan *>(j) ||
                dynamic_cast<SPTRef *>(j) || dynamic_cast<SPString *>(j));
...
std::vector<SPItem *> sp_get_same_style(SPItem *sel, std::vector<SPItem *> &src, SPSelectStrokeStyleType type)
{
    ...
    for (auto iter : src) {
        if (iter) {
            ...
            if (type == SP_STROKE_STYLE_MARKERS || type == SP_STROKE_STYLE_ALL || type == SP_STYLE_ALL) {
                ...
                for (int i = 0; i < len; i++) {
                    if (g_strcmp0(sel_style->marker_ptrs[i]->value(), iter_style->marker_ptrs[i]->value())) {
                        match = false;
                        break;
                    }
                }
            ...
            if (match_g) {
                while (iter->cloned)
                    iter = dynamic_cast<SPItem *>(iter->parent);
    ...
    if (sel_style_for_width != nullptr)
...
/**
\param  angle   the angle in "angular pixels", i.e. how many visible pixels must move the outermost point of the rotated
object
*/
void ObjectSet::rotateScreen(double angle)
{
    ...
    DocumentUndo::maybeDone(document(), ((angle > 0) ? "selector:rotate:ccw" : "selector:rotate:cw"),
                            SP_VERB_CONTEXT_SELECT, _("Rotate by pixels"));
}
...
void ObjectSet::scale(double grow)
{
    ...
    if (document()) {
        DocumentUndo::maybeDone(document(), ((grow > 0) ? "selector:scale:larger" : "selector:scale:smaller"),
                                SP_VERB_CONTEXT_SELECT, _("Scale"));
    }
}
...
void ObjectSet::scaleTimes(double times)
{
    ...
    setScaleRelative(center_, Geom::Scale(times, times));
    DocumentUndo::done(document(), SP_VERB_CONTEXT_SELECT, _("Scale by whole factor"));
}
...
void ObjectSet::move(double dx, double dy)
{
    ...
    if (document()) {
        if (dx == 0) {
            DocumentUndo::maybeDone(document(), "selector:move:vertical", SP_VERB_CONTEXT_SELECT, _("Move vertically"));
        } else if (dy == 0) {
            DocumentUndo::maybeDone(document(), "selector:move:horizontal", SP_VERB_CONTEXT_SELECT,
                                    _("Move horizontally"));
        } else {
            DocumentUndo::done(document(), SP_VERB_CONTEXT_SELECT, _("Move"));
        }
    }
}
...
void ObjectSet::moveScreen(double dx, double dy)
{
    ...
    SPDocument *doc = document();
    if (dx == 0) {
        DocumentUndo::maybeDone(doc, "selector:move:vertical", SP_VERB_CONTEXT_SELECT, _("Move vertically by pixels"));
    } else if (dy == 0) {
        DocumentUndo::maybeDone(doc, "selector:move:horizontal", SP_VERB_CONTEXT_SELECT,
                                _("Move horizontally by pixels"));
    } else {
        DocumentUndo::done(doc, SP_VERB_CONTEXT_SELECT, _("Move"));
    }
}
...
struct ListReverse
{
    ...
    static Iterator children(SPObject *o) { return make_list(o, nullptr); }
    static Iterator siblings_after(SPObject *o) { return make_list(o->parent, o); }
    static void dispose(Iterator i) { delete i; }

    static SPObject *object(Iterator i) { return *(i->begin()); }
...
template <typename D>
SPItem *next_item(SPDesktop *desktop, std::vector<SPObject *> &path, SPObject *root, bool only_in_viewport,
                  PrefsSelectionContext inlayer, bool onlyvisible, bool onlysensitive)
{
    ...
    while (!D::isNull(iter) && !found) {
        ...
        if (desktop->isLayer(object)) {
            ...
            SPItem *item = dynamic_cast<SPItem *>(object);
            if (item && (!only_in_viewport || desktop->isWithinViewport(item)) &&
                (!onlyvisible || !desktop->itemIsHidden(item)) && (!onlysensitive || !item->isLocked()) &&
...
template <typename D>
SPItem *next_item_from_list(SPDesktop *desktop, std::vector<SPItem *> const &items, SPObject *root,
                            bool only_in_viewport, PrefsSelectionContext inlayer, bool onlyvisible, bool onlysensitive)
{
    SPObject *current = root;
    for (auto item : items) {
        if (root->isAncestorOf(item) && (!only_in_viewport || desktop->isWithinViewport(item))) {
            current = item;
            break;
        }
    }
...
void sp_selection_item_next(SPDesktop *desktop)
{
    ...
    SPItem *item = next_item_from_list<Forward>(desktop, vec, root, SP_CYCLING == SP_CYCLE_VISIBLE, inlayer,
                                                onlyvisible, onlysensitive);
...
void sp_selection_item_prev(SPDesktop *desktop)
{
    ...
    std::vector<SPItem *> vec(selection->items().begin(), selection->items().end());
    SPItem *item = next_item_from_list<ListReverse>(desktop, vec, root, SP_CYCLING == SP_CYCLE_VISIBLE, inlayer,
                                                    onlyvisible, onlysensitive);
...
void ObjectSet::clone()
{
    ...
    DocumentUndo::done(document(), SP_VERB_EDIT_CLONE, C_("Action", "Clone"));
...
void ObjectSet::relink()
{
    ...
    if (!relinked) {
        ...
        DocumentUndo::done(document(), SP_VERB_EDIT_UNLINK_CLONE, _("Relink clone"));
    }
}
...
bool ObjectSet::unlink(const bool skip_undo)
{
    ...
    if (!skip_undo) {
        DocumentUndo::done(document(), SP_VERB_EDIT_UNLINK_CLONE, _("Unlink clone"));
    }
    return unlinked;
}
...
bool ObjectSet::unlinkRecursive(const bool skip_undo, const bool force)
{
    ...
    if (!skip_undo) {
        DocumentUndo::done(document(), SP_VERB_EDIT_UNLINK_CLONE_RECURSIVE, _("Unlink clone recursively"));
    }
    setList(items_);
    return unlinked;
}
...
void ObjectSet::cloneOriginal()
{
    ...
    gchar const *error =
        _("Select a <b>clone</b> to go to its original. Select a <b>linked offset</b> to go to its source. Select a "
...
void ObjectSet::toMarker(bool apply)
{
    ...
    DocumentUndo::done(doc, SP_VERB_EDIT_SELECTION_2_MARKER, _("Objects to marker"));
}
    ...
    if (items_.size() == 1) {
        ...
        if (the_group) {
            ...
            if (transform.isTranslation()) {
                ...
                // See ActorAlign::on_button_click() in src/ui/dialog/align-and-distribute.cpp
                Inkscape::Preferences *prefs = Inkscape::Preferences::get();
                int saved_compensation =
                    prefs->getInt("/options/clonecompensation/value", SP_CLONE_COMPENSATION_UNMOVED);
                prefs->setInt("/options/clonecompensation/value", SP_CLONE_COMPENSATION_UNMOVED);
...
void ObjectSet::unSymbol()
{
    ...
    if (isEmpty()) {
        if (desktop())
            desktop()->messageStack()->flash(Inkscape::WARNING_MESSAGE,
                                             _("Select a <b>symbol</b> to extract objects from."));
        return;
    }
    ...
    if (symbol == nullptr || !dynamic_cast<SPSymbol *>(symbol)) {
        if (desktop())
            desktop()->messageStack()->flash(Inkscape::WARNING_MESSAGE,
                                             _("Select only one <b>symbol</b> in Symbol dialog to convert to group."));
        return;
    }
    ...
    if (children.size() == 1) {
        ...
        if (dynamic_cast<SPGroup *>(object)) {
            if (object->getAttribute("style") == nullptr || object->getAttribute("class") == nullptr) {
    ...
    group->setAttribute("inkscape:transform-center-x", symbol->getAttribute("inkscape:transform-center-x"));
    group->setAttribute("inkscape:transform-center-y", symbol->getAttribute("inkscape:transform-center-y"));
...
void ObjectSet::tile(bool apply)
{
    ...
    gchar const *pat_id =
        SPPattern::produce(repr_copies, bbox, doc, move.inverse() /* patternTransform */, parent_transform * move);
    ...
    DocumentUndo::done(doc, SP_VERB_EDIT_TILE, _("Objects to pattern"));
}
...
void ObjectSet::untile()
{
    ...
    if (!did) {
        ...
        DocumentUndo::done(document(), SP_VERB_EDIT_UNTILE, _("Pattern to objects"));
        setList(new_select);
    }
}
...
void ObjectSet::createBitmapCopy()
{
    ...
    // Create the filename.
    gchar *const basename =
        g_strdup_printf("%s-%s-%u.png", doc->getDocumentName(), items_[0]->getRepr()->attribute("id"), current);
    ...
    // Do the export
    sp_export_png_file(doc, filepath, bbox->min()[Geom::X], bbox->min()[Geom::Y], bbox->max()[Geom::X],
                       bbox->max()[Geom::Y], width, height, res, res, (guint32)0xffffff00, nullptr, nullptr,
    ...
    if (pb) {
        ...
        DocumentUndo::done(doc, SP_VERB_SELECTION_CREATE_BITMAP, _("Create bitmap"));
    }
    if (desktop())
        desktop()->clearWaitingCursor();
...
void ObjectSet::setMask(bool apply_clip_path, bool apply_to_layer, bool skip_undo)
{
    ...
    if (apply_to_layer && is_empty) {
        ...
        if (desktop())
            desktop()->messageStack()->flash(
                Inkscape::WARNING_MESSAGE, _("Select mask object and <b>object(s)</b> to apply clippath or mask to."));
        return;
    }
    ...
    */
    ...
    for (std::vector<SPItem *>::const_iterator i = items_.begin(); i != items_.end(); ++i) {
        if ((!topmost && !apply_to_layer && *i == items_.front()) ||
            (topmost && !apply_to_layer && *i == items_.back()) || apply_to_layer) {
...
void ObjectSet::unsetMask(const bool apply_clip_path, const bool skip_undo)
{
    ...
    for (auto i = items_to_ungroup.rbegin(); i != items_to_ungroup.rend(); ++i) {
        SPGroup *group = *i;
        if (group) {
            items_to_select.erase(std::remove(items_to_select.begin(), items_to_select.end(), group),
                                  items_to_select.end());
            std::vector<SPItem *> children;
            sp_item_group_ungroup(group, children, false);
...
bool ObjectSet::fitCanvas(bool with_margins, bool skip_undo)
{
    ...
    if (bbox) {
        ...
        if (!skip_undo)
            DocumentUndo::done(document(), SP_VERB_FIT_CANVAS_TO_SELECTION, _("Fit Page to Selection"));
        return true;
    } else {
        return false;
    }
...
void ObjectSet::swapFillStroke()
{
    ...
    DocumentUndo::done(document(), SP_VERB_EDIT_SWAP_FILL_STROKE, _("Swap fill and stroke of an object"));
}
...
void verb_fit_canvas_to_drawing(SPDesktop *desktop)
{
    if (fit_canvas_to_drawing(desktop->getDocument())) {
        DocumentUndo::done(desktop->getDocument(), SP_VERB_FIT_CANVAS_TO_DRAWING, _("Fit Page to Drawing"));
    }
}
...
void fit_canvas_to_selection_or_drawing(SPDesktop *desktop)
{
    ...
    bool const changed =
        (desktop->selection->isEmpty() ? fit_canvas_to_drawing(doc, true) : desktop->selection->fitCanvas(true, true));
    if (changed) {
