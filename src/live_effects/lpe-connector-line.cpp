...
Geom::Point LPEConnectorLine::getCheckpointPosition(Geom::Curve const *previous, Geom::Curve const *curve,
                                                    Geom::Point start, Geom::Point end)
{
    auto point = curve->initialPoint();
    if (auto dynamic = getCheckpointDynamic(previous, curve)) {
 ...
 * @param curve - The curve of the checkpoint who's position is the default.
 */
int LPEConnectorLine::getCheckpointDynamic(Geom::Curve const *previous, Geom::Curve const *curve)
{
    int ret = DynamicNone;
 ...
 */
void LPEConnectorLine::setCheckpointDynamic(Geom::BezierCurve &bezier, int dynamic)
{
 ...
 */
void LPEConnectorLine::rewriteLine(SPShape *line, int index, Geom::Point p, int dir, int dynamic, int indel)
{
 ...
 * @param indel - Insertion == +1, deletion == -1, editing = 0 (default)
 */
Geom::PathVector LPEConnectorLine::rewriteLine(Geom::Path path, int index, Geom::Point p, int dir, int dynamic, int indel)
{
    Geom::PathVector pathv;
    pathv.push_back(Geom::Path());
    int j = 0;
    for (int i = 0; i < path.size(); i++) {
        if (auto bezier = dynamic_cast<Geom::BezierCurve *>(path[i].duplicate())) {
            // Upgrade bezier to quadratic bezier
            if (bezier->size() == 2) {
                bezier = new Geom::CubicBezier(bezier->initialPoint(), bezier->initialPoint(), bezier->finalPoint(),
                                               bezier->finalPoint());
            }
            ...
            if (i + 1 == index) {
                if (indel == 1) {
                    ...
                    j = 1;
                } else if (indel == -1 && index < path.size()) {
...
int LPEConnectorLine::getEndpointOrientation(Geom::Curve const *curve, bool is_end)
{
    if (auto bezier = dynamic_cast<Geom::BezierCurve const *>(curve)) {
        if (bezier->size() == 4) {
            ...
            Geom::Point adj = bezier->controlPoint((int)is_end * 3) - bezier->controlPoint(1 + (int)is_end);
            if (adj[Geom::X] > 0)
 ...
 */
Geom::PathVector connref_to_pathvector(Avoid::ConnRef *connRef, const gdouble curvature)
{
    Geom::PathVector pathv;
 ...
 */
double LPEConnectorLine::getObjectAdjustment(SPObject *line, Geom::Path path, SPItem *item)
{
    if (!item) {
        return 0.0;
    }
    ...
    Geom::CrossingSet cross = crossings(path, item_outline);
 ...
 */
LineGaps LPEConnectorLine::calculateGaps(Geom::Path input, double radius, std::vector<double> tas)
{
    auto range = input.timeRange();

    // Timecodes must be in accending order.
    ...
    for (double &ta : tas) {
        ...
        if (gaps.size() && t1 < gaps.back().second) {
 ...
 */
Geom::PathVector LPEConnectorLine::addLineJumps(SPObject *line, Geom::Path input, int type, double size)
{
    Geom::Crossings cx;
    ...
    for (auto &jump : calculateGaps(input, radius, tas)) {
                                                     ...
                                                     SPObject *target, int orientation)
{
    ...
    if (item && target) {
        ...
        pin = new Avoid::ShapeConnectionPin(shape, 1, x, y, false, 0.0, orientation);
        return shape;
    }
    return nullptr;
}
...
LPEConnectorLine::LPEConnectorLine(LivePathEffectObject *lpeobject)
    ...
    , jump_type(_("Jump type:"), _("The type of jump made when lines corss."), "jump-type", JumpType, &wr, this,
                JumpTypeArc)
...
Geom::PathVector LPEConnectorLine::doEffect_path(Geom::PathVector const &path_in)
{
    ...
    route_path = new Geom::PathVector(output);
