...
Gtk::Widget *ItemParam::param_newWidget()
{
    auto box = Gtk::make_managed<Gtk::Box>(Gtk::Orientation::HORIZONTAL);
    auto icon = Gtk::manage(sp_get_icon_image("edit-clone", Gtk::IconSize::NORMAL));
    auto button = Gtk::make_managed<Gtk::Button>();
    auto label = Gtk::make_managed<Gtk::Label>(param_label);
    label->set_hexpand();
    label->set_halign(Gtk::Align::FILL);
    box->append(*label);
    label->set_tooltip_text(param_tooltip);
    button->set_has_frame(false);
    icon->show();
    button->set_child(*icon);
    button->show();
    button->signal_clicked().connect(sigc::mem_fun(*this, &ItemParam::on_link_button_click));
    button->set_hexpand();
    button->set_halign(Gtk::Align::FILL);
    box->append(*button);
    button->set_tooltip_text(_("Link to item on clipboard"));
...
ItemParam::start_listening(SPObject * to)
{
    ...
    if (auto item = cast<SPItem>(to)) {
        linked_transformed_connection = item->connectTransformed(sigc::mem_fun(*this, &ItemParam::linked_transformed));
    }
...
ItemParam::linkitem(Glib::ustring itemid)
{
    ...
    if ( href && strcmp(itemid.c_str(), href) == 0 ) {
        ...
        DocumentUndo::done(param_effect->getSPDoc(), _("Link item parameter to path"), "");
    }
}
