...
void LPEBool::doBeforeEffect(SPLPEItem const *lpeitem)
{
    ...
    SPLPEItem *current_operand_lpeitem = dynamic_cast<SPLPEItem *>(operand_path.getObject());
    if (current_operand_lpeitem &&
        current_operand_lpeitem->hasPathEffectOfType(Inkscape::LivePathEffect::EffectType::BOOL_OP)) {
    ...
    if (desktop && operand && fork_duple) {
        fork_duple = false;
        operand_path.param_fork();
    }
    if (desktop && operand && !isOnClipboard() && fork_copy) {
        if (extra_selected) {
            desktop->selection->remove(operand);
            extra_selected = false;
        }
        fork_copy = false;
    }
...
void LPEBool::doOnFork(LivePathEffect::Effect const *forkedeffect)
{
    LivePathEffect::Effect *effect = const_cast<LivePathEffect::Effect *>(forkedeffect);
    LPEBool *original = dynamic_cast<LPEBool *>(effect);
    if (original) {
        operand = original->operand;
        sp_lpe_item = original->sp_lpe_item;
        if (original->fork_stamp) {
            operand_path.param_fork();
            original->fork_stamp = false;
            fork_stamp = true;
        }
    }
}

void LPEBool::doOnCopy(SPLPEItem const *lpeitem, Inkscape::ObjectSet *set)
{
    SPDocument *document = getSPDoc();
    if (!document) {
        return;
...
void LPEBool::doOnCut(SPLPEItem const *lpeitem) {}

void LPEBool::doOnPaste(SPLPEItem const *lpeitem)
{
    if (isOnClipboard() && sp_lpe_item) {
        operand = dynamic_cast<SPItem *>(operand_path.getObject());
        if (operand) {
            gchar *tmpid = sp_object_get_unique_id(operand, nullptr);
            operand->set(SPAttr::ID, tmpid);
            operand->updateRepr();
            Glib::ustring newid = tmpid;
            operand_path.linkitem(newid);

            const gchar *patheffectlist = sp_lpe_item->getRepr()->attribute("inkscape:path-effect");
            const gchar *lpeobjid = this->getLPEObj()->getId();
            if (lpeobjid && patheffectlist) {
                gchar *oldid = g_strdup_printf("#%s", lpeobjid);
                gchar **lpeeffects = g_strsplit(patheffectlist, ";", -1);
                Glib::ustring newpatheffectlist;
                if (lpeeffects) {
                    tmpid = sp_object_get_unique_id(this->getLPEObj(), nullptr);
                    this->getLPEObj()->getRepr()->setAttribute("id", tmpid);
                    Glib::ustring newid = "#";
                    newid += tmpid;
                    unsigned int i = 0;
                    while (lpeeffects[i]) {
                        if (g_strcmp0(lpeeffects[i], oldid) != 0) {
                            if (!newpatheffectlist.empty()) {
                                newpatheffectlist += ";";
                            }
                            newpatheffectlist += lpeeffects[i];
                        }
                        i++;
                    }
                    if (!newpatheffectlist.empty()) {
                    ...
                    newpatheffectlist += newid;
                    g_strfreev(lpeeffects);
                }
                sp_lpe_item->getRepr()->setAttribute("inkscape:path-effect", newpatheffectlist.c_str());
                g_free(oldid);
            }
            g_free(tmpid);
        }
...
void LPEBool::doOnDuple(SPLPEItem const *lpeitem)
{
    fork_duple = true;
}

void LPEBool::doOnStamp(SPLPEItem const *lpeitem)
{
    fork_stamp = true;
}
